...
static bool isInside(const int *x, const int *y, vector<vector<char>> *maze) {
    return (*x >= 0 && *y >= 0 && *x < maze->size() && *y < maze->at(*x).size());
}

static vector<Pos> *getNeighbohurs(const int *cx, const int *cy, vector<vector<char>> *maze) {
    auto *valids = new vector<Pos>();
    Pos newPos{};
    const int x[] = {0, +1, 0, -1};
    const int y[] = {+1, 0, -1, 0};
...
static vector<vector<int>> *getRiskTable(vector<vector<char>> *maze) {
    auto *risks = new vector<vector<int>>();
    for (auto &i : *maze) {
        vector<int> r;
