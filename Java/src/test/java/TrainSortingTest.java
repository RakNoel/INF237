...
public class TrainSortingTest {
    ...
    public void shortTests(){
        int[] x = {1};
        assertEquals(1, TrainSorting.LBS(x));

        int[] y = {1, 2, 3};
        assertEquals(3, TrainSorting.LBS(y));

        int[] z = {3, 2, 1};
        assertEquals(3, TrainSorting.LBS(z));

        int[] j = {1, 3, 2};
        assertEquals(2, TrainSorting.LBS(j));

        int[] p = {5, 1, 2, 3, 4, 1000, 0};
        assertEquals(5, TrainSorting.LBS(p));
    }
